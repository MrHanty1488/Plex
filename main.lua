...
return Element
    ...
    [28] = function()local maui,script,require,getfenv,setfenv=ImportGlobals(28)```lua
-- ToggleGroup.lua
local TweenService = game:GetService("TweenService")
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
function Element:New(Idx, Config)
	...
	assert(Config.Buttons, "ToggleGroup - Missing Buttons configuration")
	...
	-- Создаем ToggleFrame через Components.Element
	local ToggleFrame = require(Components.Element)(Config.Title, Config.Description, self.Container, true)
	ToggleFrame.DescLabel.Size = UDim2.new(1, - generosity, 0, 14)

	ToggleGroup.SetTitle = ToggleFrame.SetTitle
	ToggleGroup.SetDesc = ToggleFrame.SetDesc
	...
	-- Встраиваем ToggleSlider в ToggleFrame.Frame
	...
	local ButtonContainer = New("Frame", {
		...
		AutomaticSize = Enum.AutomaticSize.Y,
		LayoutOrder = ToggleFrame.Root.LayoutOrder + 1, -- Устанавливаем порядок после ToggleFrame
	}, {
		New("UIListLayout", {
			Padding = UDim.new(0, 5),
			SortOrder = Enum.SortOrder.LayoutOrder,
	...
	ToggleGroup.ButtonFrames = {}

	-- Создаем кнопки из конфигурации
	for i, ButtonConfig in ipairs(Config.Buttons) do
		local Button = require(Components.Button)(nil, ButtonContainer, false)
		Button.Title.Text = ButtonConfig.Title
		Button.Frame.LayoutOrder = i -- Устанавливаем порядок кнопок
		Creator.AddSignal(Button.Frame.MouseButton1Click, function()
			Library:SafeCallback(ButtonConfig.Callback)
			print("Button clicked:", ButtonConfig.Title)
		end)
		Button.Frame.Visible =}
		ToggleGroup.ButtonFrames[i] = Button
	end

	function ToggleGroup:OnChanged(Func)
		ToggleGroup.Changed = Func
		Func(ToggleGroup.Value)
	end

	function ToggleGroup:SetValue(Value)
		Value = not not Value
		ToggleGroup.Value = Value

		-- Обновляем визуальные элементы переключателя
		Creator.OverrideTag(ToggleBorder, { Color = ToggleGroup.Value and "Accent" or "ToggleSlider" })
		Creator.OverrideTag(ToggleCircle, { ImageColor3 = ToggleGroup.Value and "ToggleToggled" or "ToggleSlider" })
		TweenService:Create(
			ToggleCircle,
			TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
			{ Position = UDim2.new(0, ToggleGroup.Value and 19 or 2, 0.5, 0) }
		):Play()
		TweenService:Create(
			ToggleSlider,
			TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
			{ BackgroundTransparency = ToggleGroup.Value and 0 or 1 }
		):Play()
		ToggleCircle.ImageTransparency = ToggleGroup.Value and 0 or 0.5

		-- Обновляем видимость кнопок
		for _, Button in ipairs(ToggleGroup.ButtonFrames) do
			Button.Frame.Visible = ToggleGroup.Value
		end

		-- Отладочный вывод
		print("ToggleGroup SetValue:", Value, "Buttons Visible:", ToggleGroup.Value)

		Library:SafeCallback(ToggleGroup.Callback, ToggleGroup.Value)
		Library:SafeCallback(ToggleGroup.Changed, ToggleGroup.Value)
	end
	...
	Creator.AddSignal(ToggleSlider.MouseButton1Click, function()
		print("ToggleSlider clicked!")
		ToggleGroup:SetValue(not ToggleGroup.Value)
	end)

	-- Инициализируем начальное состояние
	ToggleGroup:SetValue(ToggleGroup.Value)

	Library.Options[Idx] = ToggleGroup
	return ToggleGroup
end
